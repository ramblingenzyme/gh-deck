...
describe('matchesTokens — author:', () => {
  ...
  it('no match on item without author field', () => expect(matches(baseIssue, 'author:alice')).toBe(false));
});

describe('matchesTokens — assignee:', () => {
  it('matches assignee', () => expect(matches(baseIssue, 'assignee:bob')).toBe(true));
  it('no match on item without assignee', () => expect(matches(basePR, 'assignee:bob')).toBe(false));
});
...
describe('matchesTokens — is:', () => {
  it('is:draft matches draft PR', () => expect(matches({ ...basePR, draft: true }, 'is:draft')).toBe(true));
  ...
  it('is:closed matches closed issue', () => expect(matches({ ...baseIssue, state: 'closed' }, 'is:closed')).toBe(true));
});
...
describe('matchesTokens — multiple tokens (AND logic)', () => {
  it('all tokens must match', () => expect(matches(basePR, 'repo:acme/api author:alice')).toBe(true));
  it('fails when any token does not match', () => expect(matches(basePR, 'repo:acme/api author:bob')).toBe(false));
});
